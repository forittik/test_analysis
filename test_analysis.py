...
def process_student_data_in_batches(student_data, batch_size=2):  # Reduced batch size
    ...
    return summaries  # Return list of summaries

# Generate final summary from detailed summaries
def generate_final_summary(detailed_summaries):
    # Limit the number of summaries to process at once
    if len(detailed_summaries) <= 5:  # If summaries are few, process together
        ...
        # Process in segments if too many summaries
        intermediate_summaries = []
        for i in range(0, len(detailed_summaries), 5):  # Process 5 at a time
            segment = detailed_summaries[i:i + 5]
            summary_chain = final_summary_prompt | llm | StrOutputParser()
            intermediate_summary = summary_chain.invoke({"detailed_summaries": "\n".join(segment)})
            intermediate_summaries.append(intermediate_summary)

        # Finalize the summary from intermediate results
        final_summary_chain = final_summary_prompt | llm | StrOutputParser()
        final_summary = final_summary_chain.invoke({"detailed_summaries": "\n".join(intermediate_summaries)})

    return final_summary

# Display final summary of student performance
if st.button("Generate Summary"):
    student_data_chunk = df.iloc[0:1]  # Only one student's data for summary
    detailed_summaries = process_student_data_in_batches(student_data_chunk)
    final_summary = generate_final_summary(detailed_summaries)
    st.text_area("Generated Summary", value=final_summary, height=200)
